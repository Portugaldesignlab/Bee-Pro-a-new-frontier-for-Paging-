...
export function createTextElement(
  ...
  const style = getElementStyle(config, textType)

  // Get content (from uploaded document or Lorem Ipsum)
  const content = getContentForElement(textType, width, height, style.fontSize, style.leading)
...
function getElementStyle(
  config: LayoutConfig,
  textType: "title" | "subtitle" | "body" | "caption",
  baseStyle?: Partial<ElementStyleConfig>,
): ElementStyleConfig {
  const baseFontSize = config.baseFontSize || 11
  const baseLeading = config.baseLeading || 1.4

  const styles: Record<string, ElementStyleConfig> = {
    title: {
      ...
      wordSpacing: config.wordSpacing || 100,
      textTransform: "none",
      opacity: 1,
    },
    subtitle: {
  ...
  return { ...styles[textType], ...baseStyle }
}
